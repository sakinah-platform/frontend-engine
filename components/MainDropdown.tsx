...
interface Option {
	id: number; // Ensure this is a number for consistency
	label: string;
}
...
interface DropdownProps {
	...
	idName: string;
	// values: number[]; // Multi-select: array of numbers
	values: number | null; // Multi-select: array of numbers
...
export const DropdownWithSearch: React.FC<DropdownProps> = ({
	...
	// 	// Toggle selection for the single value dropdown
	// 	setFieldValue(idName, optionId);
	// };
		...
		<div className='relative'>
			<button
				...
				<div>
					...
					<div>
						{values
							? options.find((option) => option.id === values)?.label ||
							  placeholder
							: placeholder}
					</div>
				</div>
				...
				<ul className='absolute mt-1 top-100 left-0 right-0 overflow-y-auto max-h-64 border border-primary z-50 bg-white rounded-md min-w-[200px]'>
					...
					{filteredOptions.map((option) => (
						<li
							...
							onClick={() => {
								setFieldValue(idName, option.id); // Update value on selection
								// setFieldValue; // Update value on selection
								closeDropdown(); // Close dropdown
							}}>
							<label className='ms-2 text-md lg:text-lg font-medium text-gray-900 w-full'>
								{option.label}
							</label>
						</li>
					))}
							...
							<li
								...
								<div className='flex items-center'>
									<input
										...
										className={`w-5 h-5 text-primary border-primary rounded transition focus:ring-amber-500 focus:ring-2`}
									/>
									<label
										htmlFor={targetLabel}
